...
CREATE TABLE public.users (
    ...
    VARCHAR(20) role DEFAULT 'user' CHECK (role IN ('user', 'admin', 'super_admin')),
...
BEGIN
    UPDATE public.participants 
    SET hidden_pin_hash = p_pin_hash
...
-- Tạo hàm hỗ trợ kiểm tra quyền admin (tránh lỗi vòng lặp)
...
-- 3.15 Hàm kiểm tra quyền Super Admin
...
-- ==========================================
-- PHẦN 5: TỰ ĐỘNG ĐỒNG BỘ USER TỪ AUTH SANG PUBLIC
-- ==========================================

-- Hàm xử lý khi có user mới đăng ký/đăng nhập qua Supabase Auth
CREATE OR REPLACE FUNCTION public.handle_new_user()
RETURNS trigger AS $$
BEGIN
  INSERT INTO public.users (id, username, display_name, email, avatarid, public_encrypt_key, public_sign_key, role, avatar_url)
  VALUES (
    NEW.id, 
    LOWER(SPLIT_PART(NEW.email, '@', 1)), -- Lấy phần trước @ làm username tạm
    COALESCE(NEW.raw_user_meta_data->>'full_name', SPLIT_PART(NEW.email, '@', 1)), 
    NEW.email,
    'default_avatar', -- Giá trị mặc định cho avatarid
    'no_key_yet',     -- Giá trị mặc định cho encryption keys
    'no_key_yet',
    'user',           -- Mặc định vai trò là user
    NEW.raw_user_meta_data->>'avatar_url' -- Lấy ảnh đại diện từ Google nếu có
  );
  RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
CREATE POLICY "Admins have full access to friends"
...
-- ==========================================
-- PHẦN 6: BẢNG AUDIT LOGS & PHÂN QUYỀN (SUPER ADMIN)
-- ==========================================

-- 6.1 Bảng Audit Logs (Ghi nhận mọi hoạt động trên hệ thống)
CREATE TABLE IF NOT EXISTS public.audit_logs (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES public.users(id) ON DELETE SET NULL,
  user_email TEXT,
  user_display_name TEXT,
  action_type TEXT NOT NULL CHECK (action_type IN ('login', 'logout', 'create', 'update', 'delete', 'bulk_delete', 'role_change', 'permission_change')),
  entity_type TEXT,      -- Bảng bị tác động: 'users', 'conversations', 'messages'...
  entity_id TEXT,        -- ID của bản ghi bị tác động
  details JSONB DEFAULT '{}',  -- Chi tiết: giá trị cũ/mới, mô tả hành động...
  ip_address TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW()
);

-- Indexes tối ưu truy vấn audit logs
...
-- Chỉ super_admin mới được đọc audit logs
...
-- Tất cả user đã đăng nhập đều được ghi log (để hệ thống tự ghi hành động của họ)
CREATE POLICY "Authenticated users can insert audit logs"
  ON public.audit_logs FOR INSERT
  WITH CHECK (auth.uid() IS NOT NULL);


-- 6.2 Bảng Role Permissions (Phân quyền chi tiết cho từng Admin)
CREATE TABLE IF NOT EXISTS public.role_permissions (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID NOT NULL REFERENCES public.users(id) ON DELETE CASCADE,
  resource TEXT NOT NULL,  -- Tên bảng/module: 'users', 'conversations', 'messages', 'calls'...
  can_view BOOLEAN DEFAULT true,
  can_create BOOLEAN DEFAULT false,
  can_edit BOOLEAN DEFAULT false,
  can_delete BOOLEAN DEFAULT false,
  granted_by UUID REFERENCES public.users(id) ON DELETE SET NULL,  -- Super Admin nào cấp quyền
  created_at TIMESTAMPTZ DEFAULT NOW(),
...
-- Super admin có toàn quyền quản lý permissions
...
-- Admin được đọc quyền của chính mình
CREATE POLICY "Admins can read own permissions"
  ON public.role_permissions FOR SELECT
  USING (user_id = auth.uid());

-- Trigger tự động cập nhật updated_at khi sửa quyền
...
ALTER TABLE public.audit_logs 
  ...
  CHECK (action_type IN ('login', 'logout', 'create', 'update', 'delete', 'bulk_delete', 'role_change', 'permission_change', 'permission_request'));
